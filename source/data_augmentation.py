...
import os
import numpy as np
from data import RAVIRDataset, forward_conversion, backward_conversion
import torch 
...
def resized_crop(image, mask):
    ...
    return TF.resized_crop(image, size=(768, 768), *params), TF.resized_crop(mask, size=(768, 768), *params)

def affine(image, mask):
    params = transforms.RandomAffine.get_params((-30, 30), (0.1, 0.1), (0.8, 1.2), None, (768, 768))
    return TF.affine(image, *params), TF.affine(mask, *params)

def perspective(image, mask):
    params = transforms.RandomPerspective.get_params(768, 768, 0.5)
...
def erase(image, mask):
    ...
    return TF.erase(image, *params), TF.erase(mask, *params)[..., 0].unsqueeze(-1)
...
for i in range(len(train_data)):
    ...
    mask_np = sample['mask']
    mask_conv = forward_conversion(mask_np)
    mask = torch.from_numpy(mask_conv).permute(2, 0, 1)
    ...
    for j, transformation in enumerate(transformations):
        image_tr, mask_tr = transformation(image, torch.from_numpy(mask_np.astype(np.float32)))

        mask_tr = mask_tr.numpy()
        mask_tr = (mask_tr * 255).astype(np.uint8)
        #mask_tr = backward_conversion(mask_tr)
        mask_tr = torch.from_numpy(mask_tr).permute(2, 0, 1)
        #mask_tr = transforms.ToTensor()(mask_tr).permute(1, 0, 2)
        print(mask_tr.min(), mask_tr.max())
